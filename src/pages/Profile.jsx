...
const Profile = () => {
  ...
  const [isGeneratingProfileInsights, setIsGeneratingProfileInsights] = useState(false);
  ...
  const [workoutSplitExpanded, setWorkoutSplitExpanded] = useState(false);
  const [dataManagementExpanded, setDataManagementExpanded] = useState(false);
  const [timeComparisonExpanded, setTimeComparisonExpanded] = useState(false);
  const [exportReportExpanded, setExportReportExpanded] = useState(false);

  const handleGenerateProfileInsights = async () => {
    setIsGeneratingProfileInsights(true);
    const result = await generateProfileInsights(measurements);
    setProfileInsights(result);
    setIsGeneratingProfileInsights(false);
  };

  useEffect(() => {
    handleGenerateProfileInsights();
  }, [measurements]);
  ...
  const popularSplits = {
    ...
    "full-body": {
      ...
      schedule: {
        1: {
          name: "Full Body A",
          muscles: ["chest", "back", "legs", "shoulders", "biceps"],
        },
        2: { name: "Rest", muscles: [] },
        3: {
          name: "Full Body B",
          muscles: ["chest", "back", "legs", "shoulders", "triceps"],
        },
        4: { name: "Rest", muscles: [] },
        5: {
          name: "Full Body C",
          muscles: ["chest", "back", "legs", "shoulders", "forearms"],
        },
        6: { name: "Rest", muscles: [] },
        7: { name: "Rest", muscles: [] },
      },
  ...
  useEffect(() => {
    ...
    setSelectedSplitsForPdf((prev) =>
      prev.filter((split) => splits.includes(split)),
    );
  ...
  const getFilteredWorkoutsForPdf = () => {
    if (pdfTimeRange === "0") {
      // Today only
      return workouts.filter((workout) => isToday(new Date(workout.date)));
  ...
  const getWorkoutsFromTimePeriod = (
    periodType,
    customDate = null,
    periodNumber = 1,
  ) => {
  ...
  const getTodaysPlannedWorkout = () => {
    ...
    const split =
      workoutSplit.type === "custom"
        ? workoutSplit.customSplit
        : popularSplits[workoutSplit.type];
  ...
  const formatAnalysisText = (text) => {
    ...
    return (
      text
        ...
        .replace(
          /^(🔥|💀|⚖️|🕐|📊|🎯|💡|🚨)\s*(.*?):/gm,
          '<div class="mt-6 mb-3"><span class="text-2xl mr-2">$1</span><span class="text-yellow-400 font-bold text-lg">$2:</span></div>',
        )
        // Format workout days (Week 1: **Monday: REST, etc.)
        .replace(
          /^(\*\*Week\s+\d+:\*\*)\s*(.*?)$/gm,
          '<div class="mt-4 mb-3 p-3 bg-gray-800 rounded-lg"><div class="text-purple-400 font-bold mb-2">$1</div><div class="text-gray-300">$2</div></div>',
        )
        // Format individual days within workout plans
        .replace(
          /^(Monday|Tuesday|Wednesday|Thursday|Friday|Saturday|Sunday):\s*(.*?)$/gm,
          '<div class="mb-2 p-2 bg-gray-700 rounded"><span class="text-blue-400 font-semibold">$1:</span> <span class="text-gray-200">$2</span></div>',
        )
        // Format exercise descriptions (e.g., "Bicep curls 3 sets of 8-12 reps")
        .replace(
          /([A-Za-z\s]+)\s+(\d+\s+sets?\s+of\s+\d+-?\d*\s+reps?)/gm,
          '<div class="ml-2 mb-1"><span class="text-green-400">$1</span> <span class="text-gray-400 text-sm">$2</span></div>',
        )
        // Convert bullet points
        .replace(
          /^-\s*(.*?)$/gm,
          '<div class="ml-4 mb-2 flex items-start"><span class="text-blue-400 mr-2">•</span><span>$1</span></div>',
        )
        // Format REST days
        .replace(
          /\bREST\b/g,
          '<span class="bg-orange-600 text-white px-2 py-1 rounded text-xs font-bold">REST</span>',
        )
        ...
        .replace(
          /(\d+\/10)/g,
          '<span class="bg-red-600 text-white px-2 py-1 rounded font-bold">$1</span>',
        )
        // Highlight important phrases
        .replace(
          /(MAJOR PROBLEMS|RED FLAGS|SPECIFIC IMPROVEMENTS)/g,
          '<span class="bg-red-900 text-red-200 px-2 py-1 rounded font-semibold">$1</span>',
        )
        .replace(
          /(EXCELLENT|GOOD|PERFECT)/gi,
          '<span class="bg-green-900 text-green-200 px-2 py-1 rounded font-semibold">$1</span>',
        )
        .replace(
          /(WARNING|CAUTION|AVOID)/gi,
          '<span class="bg-yellow-900 text-yellow-200 px-2 py-1 rounded font-semibold">$1</span>',
        )
        // Format notes and tips
        .replace(
          /\*\*Note:\*\*(.*?)$/gm,
          '<div class="mt-3 p-2 bg-blue-900 bg-opacity-30 border-l-4 border-blue-400 rounded"><span class="text-blue-400 font-semibold">Note:</span><span class="text-gray-300">$1</span></div>',
        )
    );
  };
  ...
  const exportProfessionalReportWithAI = async () => {
    try {
      ...
      alert(
        "Failed to generate professional report with AI analysis. Please try again.",
      );
    } finally {
      // Reset button text
      const button = document.querySelector(".export-report-ai-btn");
    ...
    try {
      ...
      const reportData = {
        ...
        summary: {
          ...
          totalExercises: filteredWorkouts.reduce(
            (sum, w) => sum + w.exercises.length,
            0,
          ),
          totalSets: filteredWorkouts.reduce(
            (sum, w) =>
              sum + w.exercises.reduce((s, e) => s + e.sets.length, 0),
            0,
          ),
            ...
            (sum, w) =>
              ...
              w.exercises.reduce(
                (s, e) =>
                  s +
                  e.sets.reduce(
                    (ss, set) => ss + (set.weight || 0) * (set.reps || 0),
                    0,
                  ),
                0,
              ),
                  ...
                  (sum, w) =>
                    ...
                    w.exercises.reduce(
                      (s, e) =>
                        s +
                        e.sets.reduce(
                          (ss, set) => ss + (set.difficulty || 0),
                          0,
                        ) /
                          Math.max(e.sets.length, 1),
                      0,
                    ) /
                      Math.max(w.exercises.length, 1),
                  0,
        ...
        workout.exercises.forEach((exercise) => {
          if (exercise.type === "cardio") {
            exercise.sets.forEach((set) => {
              ...
              reportData.cardioAnalytics[`${timing}Workout`].totalDuration +=
                duration;
              reportData.cardioAnalytics[`${timing}Workout`].totalDistance +=
                distance;
              ...
              if (
                !reportData.cardioAnalytics[`${timing}Workout`].dates.includes(
                  formattedDate,
                )
              ) {
                reportData.cardioAnalytics[`${timing}Workout`].dates.push(
                  formattedDate,
                );
              }
      ...
      const totalCardio =
        cardioData.beforeWorkout.slow +
        cardioData.beforeWorkout.fast +
        cardioData.afterWorkout.slow +
        cardioData.afterWorkout.fast;

      if (totalCardio > 0) {
        if (cardioData.afterWorkout.slow > cardioData.beforeWorkout.fast) {
          reportData.cardioAnalytics.insights.push(
            "✅ Excellent strategy: More slow cardio after workouts helps preserve muscle mass",
          );
        } else {
          reportData.cardioAnalytics.insights.push(
            "💡 Consider more slow cardio after workouts to preserve muscle mass",
          );
        }

        if (cardioData.beforeWorkout.fast > 2) {
          reportData.cardioAnalytics.insights.push(
            "⚠️ High-intensity cardio before workouts may impact strength performance",
          );
        }

        const totalDuration =
          cardioData.beforeWorkout.totalDuration +
          cardioData.afterWorkout.totalDuration;
        if (totalDuration > 180) {
          reportData.cardioAnalytics.insights.push(
            `⚠️ Excessive cardio (${totalDuration} min) may interfere with muscle growth goals`,
          );
        } else if (totalDuration > 0) {
          reportData.cardioAnalytics.insights.push(
            `✅ Good cardio volume (${totalDuration} min) for bodybuilding goals`,
          );
        }

        if (cardioData.afterWorkout.slow > cardioData.afterWorkout.fast) {
          reportData.cardioAnalytics.insights.push(
            "✅ Smart approach: Prioritizing slow cardio post-workout for recovery",
          );
        }
      }
      ...
      filteredWorkouts.forEach((workout) => {
        const workoutDate = new Date(workout.date);
        const dayName = workoutDate.toLocaleDateString("en-US", {
          weekday: "long",
        });
        ...
        workout.exercises.forEach((exercise) => {
          ...
          if (exercise.muscleGroup) {
            reportData.muscleGroupDistribution[exercise.muscleGroup] =
              (reportData.muscleGroupDistribution[exercise.muscleGroup] || 0) +
              exercise.sets.length;

            // Volume by muscle group
            const muscleVolume = exercise.sets.reduce(
              (sum, set) => sum + (set.weight || 0) * (set.reps || 0),
              0,
            );
            reportData.volumeByMuscleGroup[exercise.muscleGroup] =
              (reportData.volumeByMuscleGroup[exercise.muscleGroup] || 0) +
              muscleVolume;

            if (!reportData.summaryByMuscleGroup[exercise.muscleGroup]) {
              reportData.summaryByMuscleGroup[exercise.muscleGroup] = {
                exercises: 0,
                sets: 0,
                volume: 0,
              };
            }
            reportData.summaryByMuscleGroup[exercise.muscleGroup].exercises +=
              1;
            reportData.summaryByMuscleGroup[exercise.muscleGroup].sets +=
              exercise.sets.length;
            reportData.summaryByMuscleGroup[exercise.muscleGroup].volume +=
              muscleVolume;
          }

          // Top exercises
          reportData.topExercises[exercise.name] =
            (reportData.topExercises[exercise.name] || 0) +
            exercise.sets.length;
      ...
      const getExerciseCategory = (exerciseName) => {
        ...
        if (name.includes("rear delt") || name.includes("reverse fly"))
          return "rear_delt";
      ...
      filteredWorkouts.forEach((workout) => {
        ...
        workout.exercises.forEach((exercise) => {
          ...
          exercise.sets.forEach((set) => {
            ...
            const bodyweight =
              JSON.parse(localStorage.getItem("gymgenie-measurements"))
                ?.weight || 0;
          ...
          sessionMuscleGroups[muscleGroup].maxWeight = Math.max(
            sessionMuscleGroups[muscleGroup].maxWeight,
            exerciseMaxWeight,
          );
          ...
          sessionExercises[exerciseCategory].maxWeight = Math.max(
            sessionExercises[exerciseCategory].maxWeight,
            exerciseMaxWeight,
          );
          ...
          splits[key].muscleGroups[muscleGroup].maxWeight = Math.max(
            splits[key].muscleGroups[muscleGroup].maxWeight,
            exerciseMaxWeight,
          );
          ...
          splits[key].exercises[exerciseCategory].maxWeight = Math.max(
            splits[key].exercises[exerciseCategory].maxWeight,
            exerciseMaxWeight,
          );
          splits[key].exercises[exerciseCategory].exerciseNames.add(
            exerciseName,
          );
        });
        ...
        splits[key].maxWeight = Math.max(
          splits[key].maxWeight,
          workoutMaxWeight,
        );
      });
      ...
      Object.entries(splits).forEach(([key, split]) => {
        ...
        if (sessionCount > 0) {
          ...
          const exercisesData = {};
          Object.entries(split.exercises).forEach(
              ...
              exercisesData[exerciseCategory] = {
                ...
                displayName:
                  exerciseNames.length === 1
                    ? exerciseNames[0]
                    : `${exerciseNames[0]} (+ ${exerciseNames.length - 1} similar)`,
              };
            },
          );
          ...
          if (
            ...
            selectedSplitsForPdf.includes(key)
          ) {
          ...
          if (recentSessions.length >= 2) {
            ...
            const avgVolume =
              recentSessions.reduce((sum, s) => sum + s.volume, 0) /
      ...
      if (splitComparisonEntries.length > 0) {
        ...
        const baselineSplit = reportData.splitProgressTemp?.find(
          (s) => s.key === baselineKey,
        );
        ...
        for (let i = 1; i < splitComparisonEntries.length; i++) {
          ...
          const currentSplit = reportData.splitProgressTemp?.find(
            (s) => s.key === currentKey,
          );
      ...
      if (includeTimeComparison && reportData.timeComparison.enabled) {
            ...
            avgDifficulty:
              workouts.length > 0
                ? workouts.reduce((sum, w) => {
                    const strengthExercises = w.exercises.filter(
                      (e) => e.type === "strength",
                    );
        ...
        reportData.timeComparison.currentPeriod.data = calculatePeriodStats(
          reportData.timeComparison.currentPeriod.workouts,
        );
        reportData.timeComparison.previousPeriod.data = calculatePeriodStats(
          reportData.timeComparison.previousPeriod.workouts,
        );
        ...
        reportData.timeComparison.currentPeriod.data.muscleGroups =
          calculateMuscleGroupData(
            reportData.timeComparison.currentPeriod.workouts,
          );
        reportData.timeComparison.previousPeriod.data.muscleGroups =
          calculateMuscleGroupData(
            reportData.timeComparison.previousPeriod.workouts,
          );
      ...
      alert(
        `Professional report ${reportTypeText} generated! Open the HTML file in your browser and use Print > Save as PDF to create a PDF report.`,
      );
    } catch (error) {
...
<body>
    <div class="container">
        ...
        <div class="content">
                ...
                <div class="grid">
                    ${(() => {
                      const entries = Object.entries(
                        data.summaryByMuscleGroup || {},
                      ).sort(([a], [b]) => a.localeCompare(b));
                      if (entries.length === 0)
                        return '<div class="card">No muscle group data available for this period.</div>';
                      return entries
                        .map(
                          ([group, s]) => `
            ...
            <div class="section">
                ...
                ${(() => {
                  ...
                  splitEntries.forEach(([splitName, splitData]) => {
                    Object.keys(splitData.exercises || {}).forEach(
                      (exerciseCategory) => {
                            ...
                            <div class="card" style="margin-bottom: 20px; border-left: 4px solid #10b981;">
                                    ...
                                    ${splits
                                      .map(
                                        ...
                                        <div style="padding: 12px; background: #f8fafc; border-radius: 6px; border: 1px solid #e5e7eb;">
                                            ...
                                            ${
                                              split.data.exerciseNames.length >
                                              1
                                                ? `
                                ...
                                ${(() => {
                                  const bestVolume = Math.max(
                                    ...splits.map((s) => s.data.avgVolume),
                                  );
                                  const bestWeight = Math.max(
                                    ...splits.map((s) => s.data.maxWeight),
                                  );
                                  const bestVolumeDay = splits.find(
                                    (s) => s.data.avgVolume === bestVolume,
                                  );
                                  const bestWeightDay = splits.find(
                                    (s) => s.data.maxWeight === bestWeight,
                                  );
                    ...
                    ${Object.entries(data.splitComparison)
                      ...
                      .map(
                        ...
                        <div class="card" style="border-left: 4px solid #3b82f6;">
                            ...
                            ${
                              splitData.muscleGroups &&
                              Object.keys(splitData.muscleGroups).length > 0
                                ? `
                                <div style="margin-bottom: 15px;">
                                    <h5 style="color: #374151; margin: 0 0 10px 0; font-size: 0.9rem; font-weight: 600;">💪 Muscle Groups</h5>
                                    <div style="display: grid; gap: 8px;">
                                        ${Object.entries(splitData.muscleGroups)
                                          .map(
                                            ([muscle, data]) => `
            ...
            ${
              data.cardioAnalytics &&
              data.cardioAnalytics.beforeWorkout.slow +
                data.cardioAnalytics.beforeWorkout.fast +
                data.cardioAnalytics.afterWorkout.slow +
                data.cardioAnalytics.afterWorkout.fast >
                0
                ? `
            <div class="section">
                    ...
                    ${
                      ...
                      0
                        ...
                        <div class="card" style="border-left: 4px solid #f97316; margin-bottom: 20px;">
                            ...
                            <!-- Cardio Dates -->
                            ${
                              data.cardioAnalytics.beforeWorkout.dates &&
                              data.cardioAnalytics.beforeWorkout.dates.length >
                                0
                                ? `
                            ...
                            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap: 15px;">
                                ...
                                ${
                                  data.cardioAnalytics.beforeWorkout
                                    .totalDistance > 0
                                    ? `
                    ...
                    ${
                      ...
                      0
                        ...
                        <div class="card" style="border-left: 4px solid #10b981;">
                            ...
                            <!-- Cardio Dates -->
                            ${
                              data.cardioAnalytics.afterWorkout.dates &&
                              data.cardioAnalytics.afterWorkout.dates.length > 0
                                ? `
                                <div style="margin-bottom: 15px; padding: 10px; background: #f8fafc; border-radius: 6px; border: 1px solid #e2e8f0;">
                                    <div style="font-size: 0.8rem; color: #6b7280; margin-bottom: 5px; font-weight: 600;">📅 Cardio Sessions:</div>
                                    <div style="font-size: 0.85rem; color: #374151;">
                            ...
                            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap: 15px;">
                                ...
                                ${
                                  data.cardioAnalytics.afterWorkout
                                    .totalDistance > 0
                                    ? `
                                    <div style="text-align: center; padding: 15px; background: #f0f9ff; border-radius: 8px; border: 1px solid #bfdbfe;">
            ...
            <div class="section">
                ...
                ${
                  Object.keys(
                    data.timeComparison.currentPeriod.data.muscleGroups,
                  ).length > 0
                    ? `
                    <div style="margin-bottom: 30px;">
                        ...
                        <div class="grid" style="grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 20px;">
                            ${Object.keys(
                              data.timeComparison.currentPeriod.data
                                .muscleGroups,
                            )
                              .map((muscle) => {
                                const current = data.timeComparison
                                  .currentPeriod.data.muscleGroups[muscle] || {
                                  volume: 0,
                                  sets: 0,
                                };
                                const previous = data.timeComparison
                                  .previousPeriod.data.muscleGroups[muscle] || {
                                  volume: 0,
                                  sets: 0,
                                };
                                const volumeChange =
      ...
      reader.onload = (e) => {
        try {
          ...
          if (data.workoutSplit) {
            localStorage.setItem(
              "gymgenie-workout-split",
              JSON.stringify(data.workoutSplit),
            );
          }
  ...
  const clearAllData = () => {
    if (
      window.confirm(
        "Are you sure you want to delete ALL data including workouts, measurements, and settings? This cannot be undone.",
      )
    ) {
  ...
  const calculateTDEEWithAI = async () => {
    ...
    try {
      ...
      const bmr =
        measurements.sex === "male"
          ? 10 * parseFloat(measurements.weight) +
            6.25 * parseFloat(measurements.height) -
            5 * parseFloat(measurements.age) +
            5
          : 10 * parseFloat(measurements.weight) +
            6.25 * parseFloat(measurements.height) -
            5 * parseFloat(measurements.age) -
      ...
      const calculatedTDEE = Math.round(
        bmr * activityMultipliers[measurements.activityLevel],
      );
      ...
      const recommendedCalories =
        calculatedTDEE + goalAdjustments[measurements.goal];
        ...
        <div className="fitness-card">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="text-center p-4 bg-gray-800 rounded-xl">
              <div className="fitness-metric text-red-400">
                {stats.totalWorkouts}
              </div>
              <div className="fitness-label">Total Workouts</div>
            </div>
            <div className="text-center p-4 bg-gray-800 rounded-xl">
              <div className="fitness-metric text-green-400">
            ...
            <div className="text-center p-4 bg-gray-800 rounded-xl">
              <div className="fitness-metric text-purple-400">
                {Math.round(stats.totalCardioTime)}
              </div>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <AppleDropdown
                  ...
                  onChange={(value) =>
                    setMeasurements({ ...measurements, sex: value })
                  }
                  options={[
            ...
            <div>
              <AppleDropdown
                ...
                onChange={(value) =>
                  setMeasurements({ ...measurements, activityLevel: value })
                }
                options={[
                  {
                    value: "sedentary",
                  ...
                  {
                    value: "veryActive",
                    label: "Very Active (physical job + exercise)",
                  },
                ]}
            ...
            <div>
              <AppleDropdown
                ...
                onChange={(value) =>
                  setMeasurements({ ...measurements, goal: value })
                }
                options={[
            ...
            {calculateBMI() && (
              <div className="p-4 bg-gray-800 rounded-xl">
                <div className="flex items-center justify-between">
                  ...
                  <span className="fitness-metric-small text-blue-400">
                    {calculateBMI()}
                  </span>
                </div>
        ...
        <div className="fitness-card">
          ...
          {tdee && (
            <div className="space-y-3">
              <div className="grid grid-cols-2 gap-4">
                <div className="text-center p-4 bg-gray-800 rounded-xl">
                  <div className="fitness-metric-small text-red-400">
                    {tdee.bmr}
                  </div>
          ...
          {workoutSplit.type === "none" ? (
            <div className="text-center py-6">
              ...
              <p className="text-gray-500 text-sm">
                Choose a split to get personalized AI insights
              </p>
            </div>
          ) : (
            <div className="space-y-4">
                ...
                <div className="flex items-center justify-between mb-2">
                  <span className="text-white font-medium">
                    {workoutSplit.type === "custom"
                      ...
                      : popularSplits[workoutSplit.type]?.name}
                  </span>
                  <span className="text-blue-400 text-sm">Active</span>
                </div>
                <p className="text-gray-400 text-sm">
                  {workoutSplit.type === "custom"
                    ? workoutSplit.customSplit.description
                    : popularSplits[workoutSplit.type]?.description}
                </p>
              </div>
              ...
              <div className="bg-gradient-to-r from-blue-900/50 to-gray-800/50 border border-gray-700 rounded-2xl p-6">
  <button
    onClick={() => setAboutExpanded(!aboutExpanded)}
    className="w-full flex justify-between items-center text-left text-white p-4 rounded-lg bg-gradient-to-r from-blue-900/50 to-gray-800/50 hover:from-blue-800/50 hover:to-gray-700/50"
  >
  ...
  {aboutExpanded && (
    <div className="mt-4">
      {isGeneratingProfileInsights ? (
        <div className="text-center text-gray-400">
          <p>Generating your personalized insights...</p>
        </div>
      ) : (
        profileInsights &&
        profileInsights.success && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-gray-800/70 border border-gray-700 p-4 rounded-lg">
              <h4 className="font-semibold text-white">BMI</h4>
              <p className="text-2xl font-bold text-blue-400 break-words">
                {profileInsights.insights.bmi.value}
              </p>
              <p className="text-sm text-gray-400 break-words">
                {profileInsights.insights.bmi.interpretation}
              </p>
            </div>
            <div className="bg-gray-800/70 border border-gray-700 p-4 rounded-lg">
              <h4 className="font-semibold text-white">TDEE</h4>
              <p className="text-2xl font-bold text-green-400 break-words">
                {profileInsights.insights.tdee.value}
              </p>
              <p className="text-sm text-gray-400 break-words">
                {profileInsights.insights.tdee.interpretation}
              </p>
            </div>
            <div className="bg-gray-800/70 border border-gray-700 p-4 rounded-lg">
              <h4 className="font-semibold text-white">Healthy Weight</h4>
              <p className="text-2xl font-bold text-red-400 break-words">
                {profileInsights.insights.healthyWeight.range}
              </p>
              <p className="text-sm text-gray-400 break-words">
                {profileInsights.insights.healthyWeight.interpretation}
              </p>
            </div>
            <div className="bg-gray-800/70 border border-gray-700 p-4 rounded-lg">
              <h4 className="font-semibold text-white">Workout Frequency</h4>
              <p className="text-2xl font-bold text-yellow-400 break-words">
                {profileInsights.insights.workoutFrequency.recommendation}
              </p>
              <p className="text-sm text-gray-400 break-words">
                {profileInsights.insights.workoutFrequency.interpretation}
              </p>
            </div>
            <div className="bg-gray-800/70 border border-gray-700 p-4 rounded-lg md:col-span-2">
              <h4 className="font-semibold text-white">Macro Split</h4>
              <div className="flex justify-around mt-2">
                <div>
                  <p className="text-lg font-bold text-indigo-400">{profileInsights.insights.macroSplit.protein}</p>
                  <p className="text-xs text-gray-400">Protein</p>
                </div>
                <div>
                  <p className="text-lg font-bold text-indigo-400">{profileInsights.insights.macroSplit.carbs}</p>
                  <p className="text-xs text-gray-400">Carbs</p>
                </div>
                <div>
                  <p className="text-lg font-bold text-indigo-400">{profileInsights.insights.macroSplit.fat}</p>
                  <p className="text-xs text-gray-400">Fat</p>
                </div>
              </div>
              <p className="text-sm text-gray-400 break-words mt-2">
                {profileInsights.insights.macroSplit.interpretation}
              </p>
            </div>
            <div className="bg-gray-800/70 border border-gray-700 p-4 rounded-lg md:col-span-2">
              <h4 className="font-semibold text-white">
                {profileInsights.insights.goalAdvice.title}
              </h4>
              <p className="text-sm text-gray-400 break-words">
                {profileInsights.insights.goalAdvice.advice}
              </p>
            </div>
            {profileInsights.insights.healthTips.map((tip, index) => (
              <div key={index} className="bg-gray-800/70 border border-gray-700 p-4 rounded-lg">
                <h4 className="font-semibold text-white">{tip.title}</h4>
                <p className="text-sm text-gray-400 break-words">{tip.tip}</p>
              </div>
            ))}
          </div>
        )
        ...
        <div className="fitness-card">
          ...
          <div className="space-y-4">
              ...
              <div className="flex items-start space-x-3">
                ...
                <div className="text-sm">
                  ...
                  <p className="text-blue-300">
                    To use AI insights, you need a free Gemini API key from
                    Google AI Studio.
                    <a
                      href="https://makersuite.google.com/app/apikey"
                      target="_blank"
                      rel="noopener noreferrer"
                      className="underline hover:no-underline ml-1 text-blue-400"
                    >
                      Get your key here
                    </a>
                  </p>
            ...
            <div>
              <label className="fitness-label block mb-2">Gemini API Key</label>
              <div className="flex space-x-2">
                <input
            ...
            <div className="bg-gray-800/50 backdrop-blur-sm rounded-2xl border border-gray-700/50 overflow-hidden">
              <div className="px-6 py-4 border-b border-gray-700/50">
                ...
                <p className="text-gray-400 text-sm mt-1">
                  Generate professional reports and backups
                </p>
              </div>
              ...
              <div className="p-6 space-y-6">
                  ...
                  <div className="bg-gray-700/50 rounded-xl p-4 space-y-3">
                    ...
                    {pdfTimeRange === "custom" && pdfCustomDate && (
                      <div className="flex items-center space-x-2 text-sm">
                        ...
                        <span className="text-white font-medium">
                          {format(new Date(pdfCustomDate), "MMM d, yyyy")}
                        </span>
                      </div>
                    )}
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      onClick={() =>
                        setEnableTimeComparison(!enableTimeComparison)
                      }
                  ...
                  {enableTimeComparison && (
                    <div className="bg-gray-700/50 rounded-xl p-4 space-y-4">
                      <p className="text-gray-400 text-sm">
                        Compare your performance between different time periods
                        (e.g., "Last 30 days vs 30 days before that")
                      </p>
                      ...
                      <div className="bg-orange-900/20 border border-orange-500/30 rounded-lg px-3 py-2">
                        ...
                        <div className="text-gray-300 text-xs">
                          <div>
                            📊 Current: Last {comparisonPeriod1} days (
                            {
                              getWorkoutsFromTimePeriod(
                                comparisonPeriod1,
                                null,
                                1,
                              ).length
                          ...
                          <div>
                            📈 Previous: {comparisonPeriod2} days before that (
                            {
                              getWorkoutsFromTimePeriod(
                                comparisonPeriod2,
                                null,
                ...
                {availableSplits.length > 1 && (
                  <div className="space-y-3">
                    ...
                    <div className="bg-gray-700/50 rounded-xl p-4 space-y-4">
                      <p className="text-gray-400 text-sm">
                        Choose specific workout days/splits to compare. Leave
                        empty to include all splits.
                      </p>
                      ...
                      {selectedSplitsForPdf.length > 0 && (
                        <div className="flex items-center justify-between bg-purple-900/20 border border-purple-500/30 rounded-lg px-3 py-2">
                          ...
                          <span className="text-purple-400 font-semibold text-sm">
                            {selectedSplitsForPdf.length} split
                            {selectedSplitsForPdf.length !== 1 ? "s" : ""}
                          </span>
                        </div>
                      )}
                    </div>
                  ...
                  <div className="bg-gradient-to-r from-green-900/20 to-blue-900/20 rounded-xl p-4 border border-green-500/30">
                    <div className="flex items-center justify-between">
                      <div className="flex-1">
                        ...
                        <p className="text-gray-400 text-xs">
                          PDF with charts, analysis & AI insights for selected
                          time range
                          {selectedSplitsForPdf.length > 0 && (
                            <span className="text-green-400 font-medium">
                              {" "}
                              ({selectedSplitsForPdf.length} splits)
                            </span>
                          )}
                        </p>
                      </div>
                  ...
                  <div className="bg-green-900/20 rounded-xl p-4 border border-green-500/30">
                    <div className="flex items-center justify-between">
                      <div className="flex-1">
                        ...
                        <p className="text-gray-400 text-xs">
                          PDF with charts & basic analysis for selected time
                          range
                          {selectedSplitsForPdf.length > 0 && (
                            <span className="text-green-400 font-medium">
                              {" "}
            ...
            <div className="space-y-4">
              ...
              >
                <div className="text-left">
                  ...
                  <p className="text-gray-400 text-sm">
                    Train without a specific schedule
                  </p>
                </div>
              </button>
                            ...
                            <div className="flex flex-wrap gap-2">
                              {availableMuscles.map((muscle) => (
                                <button
                                  key={muscle}
                                  onClick={() =>
                                    toggleMuscleForDay(dayIndex, muscle)
                                  }
            ...
            <div className="prose prose-invert max-w-none">
              <div className="bg-gray-800 rounded-xl p-4 mb-4 border-l-4 border-purple-500">
                <p className="text-purple-400 font-semibold mb-2">
                  🎯 Split Being Analyzed:
                </p>
                <p className="text-white">
                  {workoutSplit.type === "custom"
                    ? workoutSplit.customSplit.name
                    : workoutSplit.type.charAt(0).toUpperCase() +
                      workoutSplit.type.slice(1).replace("-", " ")}
                </p>
              </div>
